...
class Compra:
    ...
    def obter_itens_fora_da_lista(self):
        itens_lista_base = {item.nome : True for item in self.lista_compras_base.obter_itens()}
        return [item for item in self.__itens_comprados if not itens_lista_base[item.nome]]
